...
declare @idOrder int, @orderDetailCost real, @idOrderDetail int, @username varchar(50), @idProduct int
select @idOrder = new.idOrder, @orderDetailCost = new.quantity*p.cost , @idOrderDetail = new.id, @username = _order.username, @idProduct = new.idProduct
...
begin
	...
	delete Cart
	where Cart.idProduct = @idProduct and Cart.username = @username
end
go
...
go
create trigger after_insert_Account on Account
...
go
create trigger after_update_Account on Account
after update as
declare @id int, @password_new varchar(100), @password_old varchar(100), @avatar nvarchar(max)
...
go
create trigger after_insert_update_Cart on Cart
after insert, update as
declare @id int, @cost real
select @id = new.id, @cost = new.quantity*p.cost
from inserted new, product p
